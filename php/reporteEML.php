...
function datosEstudianteMateria(){
    $con=conexion();
    $ci=$_POST['ci'];
    try {
        $sql="select ci,paterno,materno,estudiante.nombre,sigla,materia.nombre as materia
    from asignacionem
    inner join estudiante on ci=codEstudiante
...
function abrirReporte(){
    ...
    try {
        ...
        $pdf->Cell(0,10,'Estudiante: '.$row->paterno." ".$row->materno." ".$row->nombre,0,1,'L');// Agregando una celda con alineación izquierda
